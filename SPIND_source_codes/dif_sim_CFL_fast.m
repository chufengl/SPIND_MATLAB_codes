%"dif_sim_CFL_fast.m" is a new way of calculating the diffraction pattern. It is
...
% thld:
%------------------------------------
function [peak_list] = dif_sim_CFL_fast(lp,E_ph,Xsize,Msize,mos,I0,R_beam,pixelsize,L,SCA,Q,C)
 global sf_list
lamda=12.4/E_ph;
k0=1/lamda;
...
%             w=round(radius_peak/(pixelsize*1e-6));
            w=1;% to accelerate the computation, ignore the peak profile, keeping only the center.
...
Int_p_c=I0/(pi*(R_beam*1e-6)^2)*(2.82e-15)^2*sf^1*(pixelsize*1e-6/L)^2*exp(-(sg^2)/(20*r^2))*(10*Xsize/lp(1))^3;
            peak_list(row_counter,1:3)=HKL.';
            peak_list(row_counter,4:7)=[center_y_p,center_x_p,w,Int_p_c];
